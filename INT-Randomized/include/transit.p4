...
control process_int_transit (
    ...
    action int_set_header_0003_i8() {
        int_set_header_0();
        if (local_metadata.preserv_meta.sink == true) { 
            add_0();
        } else {
            add_1();
        }
    ...
    apply {
        ...
        // In case of INT-MD decrement remaining hop cnt
        if(hdr.ipv4.dscp == DSCP_INT_MD) {
            hdr.int_header.remaining_hop_cnt = hdr.int_header.remaining_hop_cnt - 1;
        }

        // Update headers lengths
        if (local_metadata.preserv_meta.sink == false) {
            if (hdr.ipv4.isValid()) {
                hdr.ipv4.len = hdr.ipv4.len + local_metadata.int_meta.new_bytes;
            }
            if (hdr.udp.isValid()) {
                hdr.udp.len = hdr.udp.len + local_metadata.int_meta.new_bytes;
            }
            if (hdr.intl4_shim.isValid()) {
                hdr.intl4_shim.len = hdr.intl4_shim.len + local_metadata.int_meta.new_words;
            }
            if (hdr.int_header.isValid()) {
                hdr.int_header.hop_metadata_len = (bit<5>) local_metadata.int_meta.new_words;
            }
        }
    }
}